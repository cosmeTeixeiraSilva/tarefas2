                ...
                <tbody>
                    {tarefas.map((tarefa, index) => (
                        <tr key={tarefa.uuid} className="text-white text-bold border border-white">
                            <td className="w-1/3 p-2 text-start text-sm">{index + 1} - {tarefa.descricao}</td>
                            <td className={`w-1/3 p-2 ${handleCor(tarefa.status)} text-black`}>
                                {tarefa.status}
                            </td>
                            <td className=" w-1/3 p-2 flex justify-around gap-x-2">
                                <>
                                    {/* Formulário para Atualizar Status */}
                                    <form action={atualizarTarefa} className="flex gap-x-8 text-sm">
                                        ...
                                        <button type="submit" className="text-green-600  bg-white p-1 rounded text-bold"> Confirmar</button>
                                        {/* Formulário para Excluir */}

                                    </form>
                                    <form action={excluirTarefa} className="text-sm p-1">
                                        <input type="hidden" name="uuid" value={tarefa.uuid} />
                                        <button type="submit" className="text-red-700"><FaRegTrashCan size={20} /></button>
                                    </form>
                                </>
